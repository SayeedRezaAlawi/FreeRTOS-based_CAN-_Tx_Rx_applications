...
uint8_t DLC; // data length cod
    union
    {
    uint8_t data_b[8]; // data as seen in bytes
    uint16_t data_h[4]; // data as seen in halfwords
    uint32_t data_w[2]; // data as seen in words
    uint64_t data_l; // data as seen in longwords
    };
} CAN_packet;
